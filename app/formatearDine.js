...
var analyzer = function (lines, prov) {
  ...
  function inicio(actual, i) {
    ...
    console.log("28", lines[i + 1] == undefined, Estados.provincia);
    //while(! lines[i++].match(Estados.provincia));
    console.log("30",lines[i + 1] == undefined);
    console.log("31",actual, i);
    return i;
  }
  ...
  function imprimirFila(actual) {
    ...
    if(actual.nombre == "GRANELLI, JUAN JAVIER"){ //.indexOf('503') != -1 && actual.nombre_lista.indexOf('DESARROLLO AGRO INDUSTRIAL') != -1){
      debugger;
    }
  ...
  function candidatos(actual, i) {
    ...
    while (!salir) {
      ...
      if(i == 380 && actual.nombre_lista.indexOf('DESARROLLO AGRO INDUSTRIAL') != -1){
        debugger;
      }
      i++;
    }
